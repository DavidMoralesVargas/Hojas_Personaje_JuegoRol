<template>
  ...
  <div class="vampire-container form-container">
    <h2>Modificar Vástago</h2>
    <hr class="divider" />

    <form @submit.prevent="editarVampiro" class="vampire-form" v-if="vampiro">
      <div class="form-group">
        <label>Nombre del Vampiro</label>
        <input v-model="vampiro.nombre" type="text" required />
      </div>

      <div class="form-group-row">
        <div class="form-group">
          <label>Clan Bane (Debilidad)</label>
          <input v-model="vampiro.clanBane.bane" type="text" />
        </div>
        <div class="form-group">
          <label>Compulsión</label>
          <input v-model="vampiro.clanBane.compulsion" type="text" />
        </div>
      </div>
        ...
        <div class="dual-list-box">
          ...
          <div class="list-box">
            ...
            <div class="items-container">
              ...
              <div v-if="disciplinasDisponibles.length === 0" class="empty-msg">No hay más disciplinas disponibles.</div>
            </div>
          </div>
      ...
      <div class="form-actions">
        ...
        <button type="submit" class="btn-blood">Guardar Cambios</button>
      </div>
    </form>
  </div>
</template>
...
const vampiro = ref(null);
const disciplinasDisponibles = ref([]);

const cargarDatos = async () => {
  const id = route.query.id; 
  if (!id) {
    router.push('/vampiros');
    return;
  }

  try {
    const todasDisciplinas = await api.get('/api/Disciplinas/combo');
    const entidadVampiro = await api.get(`/api/Vampiros/${id}`);
    
    let disciplinasAdquiridas = [];
    if (entidadVampiro.disciplinaVampiros && entidadVampiro.disciplinaVampiros.length > 0) {
      disciplinasAdquiridas = entidadVampiro.disciplinaVampiros
        .filter(dv => dv.disciplina != null)
        .map(dv => ({ id: dv.disciplina.id, nombre: dv.disciplina.nombre }));
    }

    let baneAsignado = { bane: '', compulsion: '' };
    if (entidadVampiro.debilidadesClanes && entidadVampiro.debilidadesClanes.length > 0) {
      baneAsignado = { 
        id: entidadVampiro.debilidadesClanes[0].id,
        bane: entidadVampiro.debilidadesClanes[0].bane, 
        compulsion: entidadVampiro.debilidadesClanes[0].compulsion 
      };
    }

    vampiro.value = {
      id: entidadVampiro.id,
      nombre: entidadVampiro.nombre,
      clanBane: baneAsignado,
      disciplinas: disciplinasAdquiridas
    };

    disciplinasDisponibles.value = todasDisciplinas.filter(dDisponible => 
      !disciplinasAdquiridas.some(dAdquirida => dAdquirida.id === dDisponible.id)
    );

  } catch (error) {
    console.error("Error obteniendo datos del Vástago", error);
    Swal.fire({
      icon: 'error',
      title: 'Error de Sangre',
      text: 'No se pudo cargar la información del vampiro o las disciplinas.',
      background: '#1c1c1e',
      color: '#d4d4d4',
...
const editarVampiro = async () => {
  // VALIDACIÓN: Un vampiro no puede quedarse sin disciplinas
  if (vampiro.value.disciplinas.length === 0) {
    Swal.fire({
      icon: 'warning',
      title: 'Vástago Indefenso',
      text: 'El vampiro debe conservar al menos una Disciplina adquirida en su sangre.',
      background: '#1c1c1e',
      color: '#d4d4d4',
      confirmButtonColor: '#8a0303'
    });
  ...
  try {
    await api.put('/api/Vampiros', vampiro.value);
    Swal.fire({
      icon: 'success',
      title: 'Vástago Actualizado',
      background: '#1c1c1e',
      color: '#d4d4d4',
      confirmButtonColor: '#8a0303'
    });
